...
const SIEVE_CHANGELOG = [
  {
    ...
    items: [
      ...
      "New Floating videos switch, in the same section but on its own — turning the ad blocker on does not turn this on, and it is off until you ask for it. Scroll past the video at the top of an article and most news sites shrink it into a corner and carry it down the page with you. This puts it back where the site first put it: still there, still playing if it was, just no longer in the way.",
      "It has its own switch because it is not ad blocking. The player that follows you is the site's own, sent from the site's own address, so there is no advertising address to block that would not also block the article — and wanting adverts gone is not the same as wanting video players moved.",
      "It is put back rather than closed or hidden, which matters on the sites where the video is the thing you came for. Nothing is removed and nothing is clicked on your behalf: switch this off, reload, and the page behaves exactly as the site wrote it.",
      "Sites where a floating video is the whole point are left alone — YouTube, Twitch, Netflix and the rest — and so is every video call, so a meeting cannot lose the picture of the person talking. Picture-in-picture is left alone too, because that is you asking for it.",
      ...
      "A correction, and the most important note here, because the Dark Pattern Blocker is on unless you turned it off. It matched its word lists by looking for the letters rather than the whole word, and short words hide inside longer ones: “no” sits inside Know, Economy, Ignore and Nothing, and “pass” sits inside password. So buttons that were doing nothing wrong had their words replaced with “No thanks” — “Know more about our free trial” and “Reset the password for member accounts” among them. If you ever met a button on a site that made no sense where it stood, this is the likely reason.",
      "The same mistake put the “pre-checked” badge on boxes that were nothing to do with marketing, including “I have read and accept the terms”. It also read the words around a checkbox too widely — far enough, on a simple page, to reach every other checkbox on it — so one marketing opt-in anywhere on a form marked all of them. A checkbox is now judged on the words that belong to it alone.",
      "And it stopped the cookie-banner leveller working on the commonest kind of banner. “ok” sits inside “cookie”, so a “Cookie settings” button looked like the Accept button and the Reject button at the same time; the pair then looked perfectly fair, and the real Reject — the one shrunk to a few pixels — was left exactly as the site had hidden it. Banners carrying an Accept, a settings link and a hidden Reject are handled properly now.",
      "All three are fixed, and all three now have tests, which is how they were found. These were the last three parts of Sieve that changed what a page says without anything checking that they were right to.",
      "A second correction, in the same vein: the Popup & Click Hijack Blocker has not been working on most sites. It waited for a page to stop changing before checking it, and on a page that never stops changing — a feed, a news site with a ticker, almost anything modern — it kept waiting and never checked at all. If you had that switch on, it was doing nothing on those sites and there was no sign of it. It now checks as the page goes rather than waiting for it to settle. Nothing about how it decides what to remove has changed; it simply runs where it did not before.",
      "And a security fix in the same blocker: a site could switch it off for itself — permanently, without asking, and with nothing to see afterwards. Its two halves have to talk to each other through the page, because that is the only route between them, and one of the things they said was “always allow this site”. Any page could say it too, and it wrote that site into your allow list for good. That message is gone, and nothing a page can say now changes a setting of yours. It only ever mattered if you had this blocker switched on — it is off unless you asked for it — and your allow list is untouched, though it is worth a look in the Sieve popup if you want to be sure of what is on it.",
      "That is also why “Always allow this site” has moved off the blocked-popup notice and into the Sieve popup in your toolbar, where a page cannot reach it. The notice keeps “Allow”, which lets one blocked popup through and lasts only as long as the click.",
      "Sieve is lighter on every page it runs on this release — a little under half of what it used to put there. The largest part of that is the cookie-banner rejecter, which is the biggest single piece and was being loaded onto every page whether or not you had it switched on. It is loaded when you use it and not otherwise, which is how the ad-blocking parts already worked.",
      "Long pages no longer stutter after they load. The bad-language filter reads every piece of text on a page, and the way it worked through that list got slower the further into it it got — so a long forum thread or a long article could leave the browser busy for around half a second doing nothing else. That is now effectively instant. It also skips the empty space between paragraphs, which on a long page is about two thirds of what it was reading.",
      "Scrolling is smoother on pages carrying adverts. The part that tidies away the empty boxes a blocked advert leaves behind was re-reading the entire page every time you moved, whether or not anything had appeared. It now looks only when the page actually adds something, which is the moment that matters and the only one it was there for.",
      "Tabs you are not looking at stop working in the background. A YouTube tab left open checked for adverts four times a second whether or not you were watching it, and a Facebook tab re-read the whole page every five seconds all day. Both now wait until you come back to the tab. If you keep tabs open, your battery will notice.",
      "Sieve lets go of memory it is not using. The optional comment-filtering model stayed loaded for the rest of the browsing session once anything had used it, even after every page it worked on was closed. It is now released after ten minutes unused and loaded again next time it is needed, which costs a moment on the first comment page after a break.",
      "The download is smaller: about a tenth on Chrome, and a quarter on Firefox. The Firefox version had been carrying the comment-filtering model's machinery, which Firefox has no way to run.",
      "One privacy fix came out of the same work. Two of Sieve's word lists could be requested by any page you visited. Their contents were not sensitive, but the fact that a site could ask for them at all was a reliable way to tell that Sieve was installed. They are now built into the extension, where a page cannot reach them.",
    ],
  },
  {
    version: "1.3.0",
